...
sample_data = data.sample(n=700, random_state=1)

# Contar as ocorrências de cada emoção
real_counts = sample_data['Dominant_Emotion'].value_counts().reset_index()
real_counts.columns = ['Emotion', 'Real_Count']

predicted_counts = sample_data['Predicted_Emotion'].value_counts().reset_index()
predicted_counts.columns = ['Emotion', 'Predicted_Count']

# Combinar os dados contados
emotion_comparison = pd.merge(real_counts, predicted_counts, on='Emotion', how='outer').fillna(0)

# Transformar os dados para o formato longo
emotion_comparison_long = emotion_comparison.melt(id_vars=['Emotion'], var_name='Type', value_name='Count')

# Criar o gráfico de barras
fig_comparison = px.bar(emotion_comparison_long, x='Emotion', y='Count', color='Type', barmode='group', title='Comparação entre Emoções Reais e Previstas')
